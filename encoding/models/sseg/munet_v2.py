# This is an alternative for MUNetV2 where I use second last layer as the encoded feature.
...
class MUNet(nn.Module):
    ...
    def __init__(self, n_classes=21, backbone='mobilenet_v2', pretrained=True, root='./encoding/models/pretrain', ):
        ...
        # decoder
        self.dconv4 = nn.ConvTranspose2d(in_channels=320, out_channels=96, kernel_size=4, padding=1, stride=2)
        ...
        self.invres1 = InvertedResidual(inp=32, oup=16, stride=1, expand_ratio=6)

        self.conv_out = nn.Conv2d(16, n_classes, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        y = self.conv_out(d1)  # [B, n_classes, h/2, w/2]
        y = interpolate(y, (h, w), mode='bilinear', align_corners=True)
