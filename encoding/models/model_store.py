...
def get_model_file(name, root='./encoding/models/pretrain'):
    ...
    ----------
    name : str
        Name of the model: backbone + dataname
    ...
    if name not in _model_sha1:
        ...
        model_urls = {**resnet_urls, **vgg_urls}

        if name not in model_urls:
            raise ValueError('Pretrained model for {name} is not available.'.format(name=name))
        root = os.path.abspath(root)
        return download(model_urls[name], path=root, overwrite=True)
    ...
    with portalocker.Lock(lockfile, timeout=300):
        ...
        repo_url = os.environ.get('ENCODING_REPO', encoding_repo_url)   # path of the encoding_repo
        if repo_url[-1] != '/':
            repo_url = repo_url + '/'
